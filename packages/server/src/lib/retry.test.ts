...
describe('retry helpers', () => {
  test('returns retry message for rate-limited errors', () => {
    const info = extractErrorInfo({ name: 'APICallError', message: 'Too many requests', statusCode: 429 });
  ...
  test('does not retry context overflow', () => {
    const info = extractErrorInfo({ name: 'InvalidPromptError', message: 'Request exceeds the context window.' });
  ...
  test('keeps overloaded message behavior', () => {
    const info = extractErrorInfo({ name: 'Error', message: 'Provider is Overloaded' });
