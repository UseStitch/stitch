...
describe('question service interactions', () => {
  ...
  test('askQuestion broadcasts and resolves through replyQuestion', async () => {
    ...
    await replyQuestion(questionId, [['A']]);
  ...
  test('replyQuestion accepts custom answers by default', async () => {
    ...
    await replyQuestion(question.id, [['Something else']]);
  ...
  test('replyQuestion rejects invalid answers without resolving the question', async () => {
    ...
    expect(replyQuestion(question.id, [[]])).rejects.toThrow('Question 1 requires an answer');
    expect(replyQuestion(question.id, [['Something else']])).rejects.toThrow('Question 1 received an invalid answer');
    expect(replyQuestion(question.id, [['A', 'Something else']])).rejects.toThrow('Question 1 only accepts one answer');
