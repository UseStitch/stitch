...
export async function runIncremental(ctx: MailProviderContext, provider: MailSyncProvider): Promise<IncrementalRunResult> {
  const db = getMailDb();
  const account = ctx.account;
  if (!account.syncCursor) {
    await db.update(mailAccounts).set({ syncPhase: 'backfill', updatedAt: Date.now() }).where(eq(mailAccounts.id, account.id));
  ...
  if (result.status === 'ok') {
    ...
    await db
      .update(mailAccounts)
      .set({ syncCursor: result.nextSyncCursor, syncPhase: 'incremental', lastSyncedAt: Date.now(), lastError: null, updatedAt: Date.now() })
  ...
  const touchedThreadIds = await persistSyncPage(account.id, { messages, nextPageCursor: undefined }, db);
  await db
    .update(mailAccounts)
    .set({ syncCursor: snapshot, syncPhase: 'reconciling', lastSyncedAt: Date.now(), lastError: null, updatedAt: Date.now() })
    .where(eq(mailAccounts.id, account.id));
