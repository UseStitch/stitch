// Vendored from https://github.com/fastrepl/hyprnote (crates/resampler/src/driver.rs), MIT licensed.
...
impl<R: Resampler<f32>, const CHANNELS: usize> RubatoChunkResampler<R, CHANNELS> {
  ...
  pub fn rebind_resampler(
    ...
    self.rubato_input_buffer = (0..CHANNELS)
      .map(|_| Vec::with_capacity(input_block_size.max(1)))
      .collect();
    self.rubato_output_buffer = vec![vec![0.0f32; self.resampler.output_frames_max()]; CHANNELS];
