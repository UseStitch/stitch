...
mod tests {
  ...
  // Upstream asserts 320 here, but that test is feature-gated off in their CI
  // and contradicts the implementation: for small inputs the 10ms frame needs
  // the least padding, so 160 is returned.
  #[test]
  fn test_frame_size_for_small_inputs() {
    assert_eq!(choose_optimal_frame_size(100), 160);
    assert_eq!(choose_optimal_frame_size(50), 160);
  }
}
