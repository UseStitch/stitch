...
function ReasoningBlock({ text, isStreaming }: { text: string; isStreaming?: boolean }) {
  ...
  return (
    <div className="mb-2 rounded-lg border border-border/40 bg-muted/30 overflow-hidden">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
      ...
      {open && (
        <div className="border-t border-border/40 px-3 py-2 text-xs text-muted-foreground italic">
          {text}
        </div>
      )}
    </div>
...
function ToolCallBlock({ toolName, input }: { toolName: string; input: unknown }) {
  return (
    <div className="mb-2 flex items-start gap-2 rounded-lg border border-border/40 bg-muted/20 px-3 py-2 text-xs">
...
function ToolResultBlock({ toolName, output, isError }: { toolName: string; output: unknown; isError?: boolean }) {
  return (
    <div
      className={cn(
...
function groupStoredParts(parts: StoredPart[]): DisplaySegment[] {
  ...
  for (const part of parts) {
    ...
    if (part.type === 'text-start' || part.type === 'text-end' || part.type === 'reasoning-start' || part.type === 'reasoning-end') {
      continue
    }
        ...
        {segments.map((seg) => {
          switch (seg.type) {
            ...
            case 'other': {
              ...
              switch (part.type) {
                case 'tool-call':
                  return <ToolCallBlock key={seg.key} toolName={part.toolName} input={part.input} />
                case 'tool-result': {
...
export function StreamingMessageBubble({ partIds, parts, isStreaming }: StreamingMessageBubbleProps) {
