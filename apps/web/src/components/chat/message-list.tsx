...
type RowData =
  | { kind: 'message'; id: string; role: Message['role']; parts: Message['parts'] }
  | { kind: 'compaction'; id: string; summaryParts?: Message['parts'] }
  | { kind: 'streaming' }
  | { kind: 'error'; message: string };

function buildRows(messages: Message[], streamState: ChatStreamState): RowData[] {
  const rows: RowData[] = [];
  ...
  for (const msg of messages) {
    ...
    rows.push({ kind: 'message', id: msg.id, role: msg.role, parts: msg.parts });
  }
...
export function MessageList({ messages, streamState }: MessageListProps) {
  ...
  const rows = useMemo(() => buildRows(messages, streamState), [messages, streamState]);
  ...
  const renderRowContent = (row: RowData) => {
    switch (row.kind) {
      ...
      case 'message':
        return <MessageBubble key={row.id} role={row.role} parts={row.parts} />;
      ...
      {virtualizedRowCount > 0 && (
        ...
        >
          {virtualRows.map((virtualRow) => {
            ...
            const rowKey = row.kind === 'streaming' || row.kind === 'error' 
              ? `virtual-${row.kind}` 
              : `virtual-${row.id}`;
