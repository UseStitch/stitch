    ...
    <section className="flex flex-col gap-5">
      ...
      {hasAnyMetrics ? (
        ...
        <div className="flex flex-col justify-center rounded-xl border border-border/60 bg-card p-4 shadow-sm">
          ...
          <p className={`mt-2 text-2xl font-bold ${allBlockers.length > 0 ? 'text-destructive' : 'text-foreground'}`}>
            {allBlockers.length}
          </p>
        </div>
      ...
      <div className="rounded-xl border border-border/60 bg-card p-5 shadow-sm">
        <h2 className="mb-4 text-sm font-semibold uppercase tracking-wider text-muted-foreground">Executive Summary</h2>
        {analysis?.summary ? (
          ...
          <p className="text-sm text-muted-foreground">
            {isRunning ? 'Analyzing recording to generate summary...' : 'No summary generated yet. Run the analysis to see the summary.'}
          </p>
        )}
      </div>
    </section>
