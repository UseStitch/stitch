...
export const localModelsQueryOptions = (provider: LocalProviderId) =>
  queryOptions({
    ...
    queryFn: () =>
      serverRequest<import('@stitch/shared/providers/types').LocalModel[]>(`/llm/local/${provider}/models`),
  });
...
export const discoverLocalModelsQueryOptions = (provider: LocalProviderId) =>
  queryOptions({
    ...
    enabled: false,
    queryFn: () =>
      serverRequest<import('@stitch/shared/providers/types').DiscoveredModel[]>(
        `/llm/local/${provider}/models/discover`,
      ),
  });
